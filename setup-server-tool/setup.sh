...
export_runtime() {
    ...
    export NODE_VERSIONS NGINX_MODE
}
...
Profiles:
  --all, -a                    Docker, AWS CLI, Node.js, Nginx, Certbot
...
Modules:
  preflight docker aws-cli nodejs nginx nginx-proxy-manager certbot
...
execute_request() {
    ...
    case "$ACTION" in
        ...
        security) install_security_profile ;;
        health) run_module health-check ;;
        preflight) run_module preflight ;;
        '')
